...
public class C02_actionKeyboard extends TestBase {
    @Test
    public void test01(){
        ...
        actions.keyDown(Keys.SHIFT).sendKeys("s").keyUp(Keys.SHIFT).sendKeys("amsung ").keyDown(Keys.SHIFT).sendKeys("a")
                .keyUp(Keys.SHIFT).sendKeys("71").sendKeys(Keys.ENTER).perform();
        //4- aramanin gerceklestigini test edin
        bekle(4);
        String expected="Sansung A71";
        String actualIcerik=driver.findElement(By.xpath("//h1[@class=‘a-size-base s-desktop-toolbar a-text-normal’]")).getText();

        Assert.assertTrue(actualIcerik.contains(expected));
    }
}
